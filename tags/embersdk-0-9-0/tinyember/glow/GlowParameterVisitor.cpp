...
namespace glow
{
    ...
    void GlowParameterVisitor::addProperty(gadgets::Property property, String const& value)
    {
        ...
        case Property::ValueFormat:
            m_parameter->setFormat(value);
            break;
        }
    }
    ...
    void GlowParameterVisitor::addProperty(gadgets::Property property, EnumList const& enumeration)
    {
        ...
        case Property::ValueEnumeration:
            ...
            EnumList::const_iterator first = enumeration.begin();
            EnumList::const_iterator const last = enumeration.end();
            std::list<std::pair<std::string, int> > enumMap;

            for(int index = 0; first != last; ++first, ++index)
            {
                enumMap.push_back(std::make_pair(*first, index));
            }

            m_parameter->setEnumMap(enumMap.begin(), enumMap.end());
            break;
        }
    }
}
